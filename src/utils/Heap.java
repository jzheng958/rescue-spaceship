...
/** An instance is a min heap with values of type T and priorities of type double. 
 * This is the instructors' version of a Heap. DO NOT EDIT THIS.
 */
public class Heap<T> extends ArrayList<T> {
  ...
  public void add(T item, double p) throws IllegalArgumentException {
    if (itemInfoMap.containsKey(item)) {
      throw new IllegalArgumentException(
        "Cannot add duplicate elements to the heap");
    }
  ...
  public void updatePriority(T item, double p) throws IllegalArgumentException {
    ...
    if(entry == null) 
      throw new IllegalArgumentException(
        "Cannot update priority for element not in heap");
  ...
  /** Bubble the element at index up 
   *  until it reaches its correct position in the heap.
  ...
  /** Bubble the element at index down
   *  until it reaches its correct position in the heap.
   *  Precondition: the heap satisfies all heap properties except that item
  ...
  private boolean comesBefore(int parent, int child) {
    return itemInfoMap.get(super.get(parent)).priority <=
      itemInfoMap.get(super.get(child)).priority;
  }
